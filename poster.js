...
class Poster {
  constructor(id, password) {
      ...
      const sendChar = () => {
        ...
        return new Promise(resolve => {
          if (idx < fullContent.length) {
            ...
            this.stream.write(fullContent[idx])
            idx++
      ...
      switch (this.currentState) {
        ...
        case status.readArticle:
          ...
          if(match) {
            ...
            if(article) {
              if(this.isNeedBackup) writeFile(article.content, `./backup/${this.board.toLowerCase()}-${this.articleNumber}`) 
     ...
     */
    postArticle = (options) => {
      const { board, title, articleNumber, contentPath, target, stance, isSendByWord, isNeedBackup } = options 
      ...
      return new Promise((resolve, reject) => {
        ...
        this.conn.on('ready', () => {
          ...
          this.conn.shell({
            pty: { term: 'vt100', cols: 80, rows: 24, width: 640, height: 480 }
          }, (err, stream) => {
            ...
            stream.on('close', () => {
              ...
              if (this.currentState !== status.postDone) {
                reject(new Error('Connection closed unexpectedly before post done.')) 
              }
            }).on('error', (err) => {
